def solution(s):
    ...
    for zip_size in range(1, max_zip+1):
        # zip_size에 할당된 압축크기로 문자열을 분할해서 리스트로 반환
        splited = [s[i:i+zip_size] for i in range(0, LENGTH, zip_size)]
        ...
        for i in range(len(splited)-1):
            # 현재 분할문자와 다음 분할문자를 비교하기 위해 변수로 할당
            cur, nex = splited[i], splited[i+1]
            # 만약 현재 분할문자와 다음 분할문자가 같다면 문자의 반복 횟수(count)를 1 추가함
            if cur == nex:
                count += 1
            # 다르다면 현재까지 반복된 횟수를 분할문자와 함께 string에 이어붙임
            else:
                string += (str(count) if count != 1 else "") + \
                    cur  # 반복횟수(count)가 1이라면 분할문자만 이어붙임
                count = 1  # 반복횟수를 초기화

        string += (str(count) if count != 1 else "") + \
            nex  # 마지막 인덱스의 분할문자를 string에 이어붙임
        # 위에서 압축할때 count가 초기화 된 상태라면 분할문자만,
        # count가 초기화되지 않았다면 count와 함께 분할문자를 이어붙임
        # 압축된 문자열의 길이를 cand리스트(압축크기별 문자열 길이 보관 리스트)에 append
        cand.append(len(string))
